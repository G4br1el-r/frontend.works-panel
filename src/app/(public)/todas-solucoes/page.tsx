...
export default function AllSolutionsPage() {
  return (
    <main className="w-full">
      <SolutionHero title="TODOS OS SERVIÇOS" image={SOLUTIONS_MORE_CARD.image} />
          ...
          {SOLUTIONS_ITEMS.map((solution, index) => (
            ...
            >
              <div className="flex flex-col gap-6 sm:gap-8">
                <SectionHeading eyebrow={`SOLUÇÃO ${String(index + 1).padStart(2, "0")}`} lines={[solution.title]} />

                <p className="max-w-3xl text-sm leading-relaxed text-pretty text-white/70 sm:text-base">
                  {solution.description}
                </p>

                <SolutionServices servicos={solution.servicos} solutionLabel={solution.label} />
              </div>
            </FadeIn>
          ))}
        </div>
