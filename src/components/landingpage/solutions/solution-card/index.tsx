...
export function SolutionCard({ id, title, description, image, index }: SolutionCardProps) {
  return (
    <Link
      href={`/solucoes/${id}`}
      className="group relative flex w-full overflow-hidden bg-black h-64 sm:h-72 md:h-80 xl:h-96"
    >
      <ImageComponent
        src={image}
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col gap-2 p-4 transition-transform duration-500 ease-out group-hover:-translate-y-2 sm:gap-3 sm:p-5 md:p-6">
        ...
        <span className="inline-flex items-center gap-2 text-[0.7rem] font-bold tracking-widest text-brand-light transition-colors group-hover:text-white sm:text-xs">
          VER SERVIÇOS
          <ArrowRight
            size={14}
